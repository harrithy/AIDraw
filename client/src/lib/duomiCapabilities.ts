...
export const DUOMI_CAPABILITIES: DuomiCapability[] = [
  ...
  capability({ id: "image.gpt-image-2", name: "GPT Image 2", provider: "GPT Image", category: "image", status: "released", outputKind: "image", sourceDocId: "api-192667743", priceLabel: "约 ¥0.06/次", priceNote: "Sunburst 为官方暂定单价", create: { path: "/v1/images/generations?async=true" }, query: { strategy: "path", method: "GET", path: "/v1/tasks/{id}" }, fields: [select("model", "模型", ["gpt-image-2", "gpt-image-2.5-flare", "gpt-image-2.5-sunburst"], "gpt-image-2", true), prompt(), select("size", "尺寸", ["auto", "1024x1024", "1792x1024", "1024x1792", "1:1", "3:2", "2:3", "16:9", "9:16", "1:2", "2:1", "4:3", "3:4", "5:4", "4:5"], "auto"), { key: "image", label: "参考图片 URL", type: "url-list" }, select("quality", "质量", ["low", "medium", "high"], "high"), bool("oversea", "海外线路"), url("callback_url", "回调 URL", false)] }),
  ...
  capability({ id: "music.suno.generate", name: "SUNO 生成音乐", provider: "SUNO", category: "music", status: "released", outputKind: "music", sourceDocId: "api-192667789", priceLabel: "约 ¥0.34/次", create: { path: "/api/suno/generate", contentType: "multipart/form-data" }, query: sunoQuery, fields: [url("callback_url", "回调 URL", false), select("make_instrumental", "纯音乐", [0, 1], 0, true), prompt(false), select("mv", "版本", ["chirp-bluejay", "chirp-auk", "chirp-v4", "chirp-v3-5", "chirp-fenix"], "chirp-fenix"), text("title", "歌名"), text("tags", "风格"), text("negative_tags", "排除风格"), text("gpt_description_prompt", "GPT 描述提示词"), json("metadata", "音乐控制参数"), text("continue_at", "续写起点（秒）"), text("continue_clip_id", "续写片段 ID"), text("cover_clip_id", "翻唱片段 ID")] }),
