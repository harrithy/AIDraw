...
export const INITIAL_RELEASES: ReleaseNote[] = [
  {
    version: "v1.4.16",
    title: "全局图片与视频预览视野拓宽、大尺寸原画高清呈现",
    date: "2026-09-23",
    badge: "✨ 最新版本",
    summary:
      "v1.4.16 大幅拓宽了全站所有媒体与参考图预览弹窗的宽高限制：主大图预览最大宽度由 960px 扩充至 1440px（提升 50%），内部图片与视频渲染最大高度上限由 820px 提升至 1080px (86vh)（提升 31%）；同时将卡片与创作面板的参考图弹窗起步宽度由 520px 扩展至 760px，上限支持拉伸至 1360px；素材库预览上限同步提至 1360px，版本对比视图拓宽至 1500px，并收紧了遮罩安全边距，让高分辨率艺术画作与参考细节一览无余。",
    highlights: [
      "🖥️ 预览大视界：主弹窗最大宽度由 960px 扩展至 1440px，高分屏下超大舒展呈现",
      "🖼️ 原画高清高度：图片与视频最大渲染高度提升至 1080px (86vh)，纵向画面细节纤毫毕现",
      "🎨 参考图视野大开：任务卡片与创作面板参考图弹窗宽度大幅跃升至 760px ~ 1360px，告别小卡片局促感",
      "⚖️ 素材库与对比视图拓宽：素材库预览上限扩至 1360px，版本对比视图拓宽至 1500px",
      "📐 边缘利用率最大化：收紧背景遮罩留白，释放更多宝贵的屏幕面积供画面展示"
    ],
    items: [
      {
        category: "improvement",
        title: "全局预览弹窗宽度与高度上限扩充",
        description:
          "主预览模态框（ImagePreview）最大宽度从 960px 提升至 1440px，最小宽度从 480px 提升至 640px；内部媒体画面最大高度限制从 820px (76vh) 提升至 1080px (86vh)，大屏浏览视觉冲击力显著增强。",
        tag: "视觉升级"
      },
      {
        category: "improvement",
        title: "卡片与创作面板参考图弹窗尺寸扩大",
        description:
          "参考图放大弹窗（JobCard / CreateJobPanel）起步宽度由 520px 提升至 760px，最大宽度支持至 1360px，参考图点击后能以开阔尺寸清晰呈现画面细节。",
        tag: "参考图优化"
      },
      {
        category: "improvement",
        title: "素材库与对比视图大屏适配",
        description:
          "全局与文件夹素材库预览宽度上限提升至 1360px，多版本对比视图宽度上限提升至 1500px；背景遮罩四周留白由固定 32px 优化为响应式 min(20px, 2.5vw)，充分利用显示器全屏视野。",
        tag: "空间释放"
      }
    ]
  },
  {
    version: "v1.4.15",
    title: "混合参考图顺序保真、附件身份稳定与后台上传逻辑可测化",
    date: "2026-09-22",
    summary:
      "v1.4.15 打磨参考图后台上传的细节：当参考图中同时存在「已上传的公网图片」与「正在上传的图片」时，上传结果会按原顺序原位回填，不再打乱多图参考的排列或顶替掉已有图片；附件身份改用稳定标识，同一张图片重复添加也能精准单独移除；同时把待上传参考图的解析逻辑抽成独立纯函数并补齐单元测试，覆盖顺序回填、占位失配与上传失败等边界。",
    highlights: [
      "🧩 混合参考图顺序保真：公网图与上传中图片混排时原位回填，多图参考顺序与数量与选择时完全一致",
      "🆔 附件身份稳定：参考图改用稳定标识作为键值，重复添加同一张图（或同一 URL）也能精准单独移除",
      "🧪 上传解析逻辑可测化：待上传参考图解析抽为独立纯函数，新增单元测试覆盖原位回填、占位失配、上传失败与旧记录兼容",
      "🖼️ 画布参考图如实呈现：任务卡片的参考图条不再对重复地址做去重，与创作面板中的实际参考图张数保持一致",
      "🛡️ 上传中断判定复用同一入口：队列判定「上传中断」与提交前解析共用同一套判断，避免两处逻辑漂移"
    ],
    items: [
      {
        category: "fix",
        title: "混合参考图的原位回填与顺序保真",
        description:
          "修复同时存在已上传公网图片与上传中图片时，上传结果会把图片整体前移、顶替原有图片位置的问题。现在按占位地址逐张原位回填，既保留公网图片，也保持参考图的选择顺序与总张数。",
        tag: "顺序修复"
      },
      {
        category: "fix",
        title: "参考图附件稳定身份与精准移除",
        description:
          "参考图附件引入稳定标识作为列表键值与移除依据，替代原先以地址判定身份的做法。重复添加同一张图片或同一个 URL 时不再互相顶替，移除操作精确命中目标附件，上传中的附件也能被正确注销。",
        tag: "交互修复"
      },
      {
        category: "improvement",
        title: "待上传参考图解析逻辑抽离与可测化",
        description:
          "把原先内嵌在任务调度流程中的参考图解析逻辑抽为独立的纯函数模块：统一处理多图占位回填、旧版仅登记 Key 的任务记录以及上传中断判定，并补齐单元测试覆盖顺序回填、占位失配、上传失败与兼容分支。",
        tag: "可测试性"
      },
      {
        category: "improvement",
        title: "任务卡片参考图如实呈现",
        description:
          "任务卡片的参考图条不再对重复地址做去重，改为按任务记录中的实际参考图逐张呈现，与创作面板所选参考图的数量与顺序保持一致。",
        tag: "呈现优化"
      }
    ]
  },
  {
    version: "v1.4.14",
    title: "参考图后台上传、加入队列零等待与上传中断保护",
    date: "2026-09-22",
    badge: "✨ 最新版本",
    summary:
      "v1.4.14 把参考图上传从「加入队列」里彻底解耦：选好图片即刻发起后台上传并立即入队，无需盯着进度条等待，加入队列按钮与附件区不再因上传被禁用；上传中的缩略图显示进度遮罩，上传完成自动回填图床公网地址。若上传途中刷新或断网，任务会被安全标记为「上传中断」并给出重试提示，不会带着无效地址提交或重复计费。",
    highlights: [
      "⚡ 加入队列零等待：参考图改为后台上传，选完图直接点「加入队列」，按钮不再因上传被禁用",
      "🖼️ 缩略图上传进度可视：上传中的附件显示进度遮罩与「上传参考图中...」状态，完成即自动替换为图床地址",
      "🔗 任务携带上传凭据：任务记录保存待上传标识，调度器执行到该任务时自动等待上传完成并回填真实公网 URL",
      "🛡️ 上传中断保护：刷新页面或断网导致上传未完成时，任务被标记为「上传中断」并提示重试，而不是误报重复计费风险",
      "🧹 附件移除即释放：移除参考图会同步注销后台上传与本地预览资源，避免无效上传与内存占用"
    ],
    items: [
      {
        category: "feature",
        title: "参考图后台上传与入队解耦",
        description:
          "新增前端上传注册池（uploadRegistry）：选图后立即发起图床上传并向注册池登记，加入队列时无需等待上传完成。任务记录新增 pendingUploadKeys 字段保存待上传标识，调度器执行到该任务时自动等待上传完成并回填真实公网 URL，再提交给服务端。",
        tag: "上传重构"
      },
      {
        category: "feature",
        title: "附件上传中的可视化状态",
        description:
          "上传中的参考图缩略图叠加半透明进度遮罩与旋转图标，鼠标悬停提示「后台上传中...」；上传成功后自动替换为图床地址，画布上的任务盒子同步显示「上传参考图中...」状态，全流程进度一目了然。",
        tag: "状态可视"
      },
      {
        category: "fix",
        title: "上传中断的安全标记与重试引导",
        description:
          "当页面刷新或网络中断导致后台上传无法完成时，任务不再以「提交状态未知」笼统失败，而是标记为「上传中断」并提示「参考图上传未能完成，请重试」，避免用户误以为发生了重复计费。",
        tag: "异常处理"
      },
      {
        category: "improvement",
        title: "多图上传计数与独立移除",
        description:
          "参考图上传进度由布尔开关改为计数维护，多张图片并发上传时进度互不干扰；每张附件可单独移除，移除时同步注销对应的后台上传任务，已取消的图片不再继续占用网络与内存。",
        tag: "并发优化"
      },
      {
        category: "fix",
        title: "本地预览资源释放与主题配色修正",
        description:
          "移除参考图或关闭创作面板时主动释放本地预览用的 object URL，避免长时间会话下内存累积；上传进度遮罩的强调色改用项目主题令牌，深色、浅色与动漫主题下配色统一。",
        tag: "细节修复"
      }
    ]
  },
  {
    version: "v1.4.13",
    title: "全局任务搜索上线、跨文件夹一键跳转与画布智能定位",
    date: "2026-09-22",
    badge: "✨ 最新版本",
    summary:
      "v1.4.13 将顶部搜索升级为跨文件夹全局任务搜索：输入提示词、负向提示词、任务状态、任务 ID 甚至文件夹名都能秒级命中全站任意盒子，结果卡片直接标注所属文件夹并支持一键复制；点击或回车即可跳转，目标任务在当前文件夹时直接居中定位，位于其它文件夹则自动切换文件夹并平滑平移居中；同时修正搜索相关的样式与定位边角问题。",
    highlights: [
      "🔎 全局任务搜索：突破单文件夹限制，一次检索全站所有文件夹下的历史盒子，冷门旧作也能瞬间找回",
      "🏷️ 结果标注归属文件夹：每条结果携带文件夹徽标，跨项目同名提示词一眼分清来源",
      "🎯 一键跳转与智能居中：点击结果即跳转，同文件夹直接居中；跨文件夹自动切换并等卡片渲染完成后平移定位",
      "⌨️ 回车直达首条结果：键盘输入后直接回车，无需鼠标即可跳到最匹配的盒子",
      "🧹 搜索面板样式与定位修正：文件夹徽标改用主题令牌配色，手动切换文件夹时不再残留延迟跳转"
    ],
    items: [
      {
        category: "feature",
        title: "跨文件夹全局任务搜索",
        description:
          "顶部搜索框由「仅搜当前文件夹」升级为全站检索，覆盖所有文件夹下的历史任务。匹配范围为提示词、负向提示词、任务状态、任务 ID 与所属文件夹名，支持空格分词的多关键词模糊匹配，任意关键词命中即可展示。",
        tag: "搜索升级"
      },
      {
        category: "feature",
        title: "结果归属标注与一键复制",
        description:
          "每条搜索结果新增文件夹徽标，直接显示该盒子所属的文件夹名称，长文件夹名自动省略并以悬停提示补全；结果卡片同时保留缩略图、生成时间与右侧一键复制提示词按钮。",
        tag: "结果呈现"
      },
      {
        category: "feature",
        title: "点击 / 回车跳转与跨文件夹画布定位",
        description:
          "点击搜索结果或直接按回车即可跳转到目标盒子：目标位于当前文件夹时立即平移居中；位于其它文件夹时先自动切换文件夹，待新画布卡片布局渲染完成后自动居中，并提示已切换并定位到任务。",
        tag: "跳转定位"
      },
      {
        category: "fix",
        title: "搜索面板文件夹徽标配色修正",
        description:
          "修正文件夹徽标使用的未定义样式变量，改用项目主题令牌着色，徽标背景与文字在深色、浅色与动漫主题下均正确呈现，不再出现背景透明、文字颜色与主题脱节的问题。",
        tag: "样式修复"
      },
      {
        category: "fix",
        title: "跨文件夹跳转残留与全局列表同步修复",
        description:
          "跨文件夹定位期间若手动切换到其它文件夹，不再残留延迟跳转状态，避免稍后返回时画布意外自动平移；同时删除盒子与清理失败盒子后立即同步全局搜索列表，防止搜索结果仍指向已删除的任务。",
        tag: "定位修复"
      }
    ]
  },
  {
    version: "v1.4.12",
    title: "全局总素材库上线、一键设为参考图与网络代理全面兼容",
    date: "2026-09-21",
    badge: "✨ 最新版本",
    summary:
      "v1.4.12 重磅上线全局总素材库，支持全站素材与按文件夹分类一览、图片全幅饱满呈现与一键添加为当前会话参考图；去除了大图预览冗余信息与多余关闭按钮，实现极致沉浸体验；全面兼容 Clash TUN / Fake-IP 模式代理链路，彻底解决本地图片上传网络拦截问题。",
    highlights: [
      "🗂️ 全局总素材库上线：顶部工具栏一键开启，聚合全站素材图片，支持按文件夹快捷切换与全幅网格呈现",
      "🎯 参考图一键极速注入：素材卡片悬停操作重构，右下角点击「用作参考图」直接添加至当前文件夹对话框",
      "🖼️ 纯净大图预览与去噪：精简素材库弹窗与大图预览底栏，去除非必要控制按钮，支持遮罩与 Esc 快捷退出",
      "🛡️ Clash 代理链路全面兼容：精准放行 198.18.0.0/15 Fake-IP 网段，彻底解决开启 Clash 代理时上传图片报 400 拦截的问题",
      "🧹 素材卡片操作精准布局：删除图标固定于右上角、参考图图标固定于右下角，布局清晰利落防误触"
    ],
    items: [
      {
        category: "feature",
        title: "全局总素材库中心实装",
        description:
          "顶部主工具栏新增独立总素材库入口，跨文件夹汇聚全部生成与上传素材。支持全量/分文件夹过滤，图片自适应卡片比例全幅展现，告别黑边与空白。",
        tag: "素材管理"
      },
      {
        category: "feature",
        title: "一键设为当前创作参考图",
        description:
          "素材悬停右下角点击「用作参考图」，瞬间静默添加至当前活跃文件夹的创作对话框中，无需繁琐复制下载，交互行云流水。",
        tag: "交互提升"
      },
      {
        category: "improvement",
        title: "素材操作卡片精准防误触布局",
        description:
          "重构素材卡片悬停操作层：右上角为危险操作「删除」，右下角为高频操作「用作参考图」，对角线分离布局有效防止误触并提升操作直觉。",
        tag: "布局重构"
      },
      {
        category: "improvement",
        title: "纯净大图预览与界面去噪",
        description:
          "素材卡片点击大图预览彻底移除底部多余信息区域，移除冗余的关闭 X 按钮，保留最纯粹的高清看图体验，支持遮罩与 Esc 极速关闭。",
        tag: "UI精简"
      },
      {
        category: "fix",
        title: "Clash TUN / Fake-IP 代理兼容与上传修复",
        description:
          "优化服务端 SSRF 安全拦截规则，安全放行 198.18.0.0/15 虚拟保留网段，彻底解决在开启 Clash TUN 或 Fake-IP 模式时远程图片上传报 400（禁止访问私网保留地址）的异常。",
        tag: "网络修复"
      }
    ]
  },
