...
- **📋 タスクキュー管理** — 最大30件の同時実行、非同期ステータスの自動ポーリング、失敗タスクのリトライ
...
コアはブラウザ側 SPA ですが、クロスオリジンのメディア取得と転送には `/api/media-proxy`、`/api/media-upload` の軽量関数を使用します。他のプラットフォームでは SPA フォールバックに加えて同等の安全なプロキシが必要です。
