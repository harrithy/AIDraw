...
import { generatedDir } from "./db.js";
...
const paletteFromPrompt = (prompt) => {
...
export const simulateDrawing = async (job) => {
  const duration = 1800 + Math.floor(Math.random() * 2600);
  ...
  const fileName = `${job.id}-${nanoid(6)}.svg`;
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  <text x="${width * 0.1}" y="${height * 0.28}" fill="rgba(255,255,255,0.88)" font-family="Segoe UI, sans-serif" font-size="${Math.max(14, width * 0.03)}">Mock AI output</text>
  ...
  writeFileSync(absolutePath, svg.trim(), "utf8");
