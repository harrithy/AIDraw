...
一个以浏览器本地数据为核心的多模态 AI 创作工作流管理器，支持图片、视频、音频、文本和文件类能力，以文件夹组织任务，并在可缩放画布上展示结果。
...
- **🧰 多模态能力库** — 除文生图 / 图生图外，还支持视频、音频、文本、文件等注册表能力
- **📋 任务队列管理** — 最多 30 个任务并发，自动轮询异步任务状态，支持重试失败任务
...
- **💾 Local-first 持久化** — 文件夹、任务、设置和媒体链接保存在浏览器 IndexedDB；媒体原文件仍由对应远程服务保存
...
核心应用是浏览器端 SPA，但下载跨域媒体和远程媒体转存使用了 `/api/media-proxy`、`/api/media-upload` 两个轻量函数。部署到其他平台时，除 SPA 路由回退外，还需要提供等价的安全代理；若只部署静态文件，这两项辅助能力不可用。
...
顶栏可以导出 / 导入文件夹 JSON。备份包含任务参数、画布状态、结果历史和素材记录，但只保存媒体 URL，不包含图片或视频原文件；长期归档时请同时单独保存重要媒体。

---

## 🤝 贡献

欢迎提交 Issue 和 Pull Request！在提交代码前请确保通过类型检查和测试：
